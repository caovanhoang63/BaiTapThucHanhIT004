...
CREATE OR ALTER TRIGGER INSERT_HOADON_CAU14 ON HOADON
    FOR UPDATE
    AS
    BEGIN
        DECLARE @sohd INT
    ...
    SELECT * FROM CTHD
CREATE OR ALTER TRIGGER UPDATE_CTHD_CAU14 ON CTHD
    FOR UPDATE
    AS
    BEGIN
        ...
        SELECT @GTM = @SLM * @GIA
        UPDATE HOADON
        SET TRIGIA = TRIGIA = TRIGIA - @GTC + @GTM
        WHERE SOHD = @sohd
    END

CREATE OR ALTER TRIGGER INSERT_CTHD_CAU14 ON CTHD
    FOR
    INSERT
    AS
    BEGIN
...
CREATE OR ALTER TRIGGER DELETE_CTHD_CAU14 ON CTHD
    FOR
    DELETE
    AS
    BEGIN
